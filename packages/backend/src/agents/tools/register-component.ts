...
export const registerComponentTool = tool(
  async ({ deckId, name, version, description, parameters, expansion }) => {
    ...
    return JSON.stringify({
      ...
      message: `Registered component "${name}" v${version} for deck ${deckId}`,
    });
  },
  {
    name: 'register_component',
    description: `Register a new reusable component for this deck. Use this when you've created a pattern that could be reused across multiple slides.
...
The expansion template uses JSX-like syntax with {params.paramName} for parameters and {children} for nested content.`,
    schema: z.object({
      deckId: z.string().describe('The deck ID to register the component for'),
      name: z
        .string()
